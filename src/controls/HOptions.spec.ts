...
describe("HOptions", () => {
  ...
  it.each([
    ...
    await at(wrapper.findAll('[role="radio"]'), 1).trigger("keydown", { key });
  ...
  it.each([
    ...
    await at(wrapper.findAll('[role="radio"]'), from).trigger("keydown", { key });
  ...
  it("ignores keys that are not group navigation", async () => {
    ...
    await at(wrapper.findAll('[role="radio"]'), 1).trigger("keydown", { key: "a" });
  ...
  it("moves focus along with the selection", async () => {
    ...
    await at(radios, 1).trigger("keydown", { key: "ArrowRight" });
    await wrapper.setProps({ modelValue: 2 });

    expect(document.activeElement).toBe(at(radios, 2).element);
    wrapper.unmount();
  });
});
