...
| 2026-09-11 | `headsetcontrol` **4.1.0** wydane (2026-08-27) — niesie obie nasze upstreamowe poprawki wydajności ([#549](https://github.com/Sapd/HeadsetControl/pull/549), [#550](https://github.com/Sapd/HeadsetControl/pull/550)). `MIN_VERSION` = **4.1.0**, i tym razem powodem jest **czas odpowiedzi, nie wsparcie sprzętu**: zapis parametru przez aplikację kosztował na 4.0.0 **2,90 s**, na 4.1.0 kosztuje **0,07 s** (Maxwell 2, trzy przebiegi). 4.0.0 nadal działa — po prostu na tyle wolno, że aplikacja zachowuje się wyraźnie gorzej, a nie ma jeszcze wydania, które kogoś odcina. `api_version` CLI idzie 1.4 → 1.5, bo zapis nie raportuje już wartości info. Ekrany instalacji prowadzą teraz najpierw do **repozytoriów** upstreamu (PPA / COPR / AUR), potem do podpisanych pakietów, a budowanie ze źródeł zostaje na końcu. **Ograniczenie sprzętowe:** 4.1.0 nadal wysyła przy każdym odczycie info pakiet zapisujący parametr DSP, który trwale przesuwa balans słuchawek ([#561](https://github.com/Sapd/HeadsetControl/issues/561), usunięty w [#577](https://github.com/Sapd/HeadsetControl/pull/577) już po tagu) — fixtures czytające prawdziwy zestaw nagrywamy z builda mastera, nigdy z wydania — [ADR 0010](decisions/0010-binary-detection-and-permission-diagnosis.md) |
